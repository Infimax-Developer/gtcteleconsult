				...
				>
					{contents.map(({ title, i }, index) => (
						...
						>
							{({ selected }) => (
								<div
									className={`outline-0 px-3 mx-0 py-2 duration-200 text-base rounded-lg flex gap-2 justify-center items-center bg-white cursor-pointer hover:bg-primary/[0.1] rounded-xl ${
										selected
					...
					<Tab.Panel
						...
						className={`p-3 duration-200 text-base rounded-xl flex flex-col  overflow-auto ${contentClassName} ${
							contentClassName.includes("max-h")
								? ""
								: " max-h-[calc(100vh-388px)]   min-h-[288px]"
						}`}
					>
